...
@php
    ...
    $existingKebutuhanJson = old('kebutuhan_data_json', $respondent->kebutuhan_data_json ?? '[]');
    // Tambah ini:
...
<script>
    // Value lama dari dropdown fasilitas q8
    const oldFasilitas = "{{ $oldFasilitas }}";
</script>
...
<script>
    ...
    function nextBlock(nextBlockNumber) {
        if (validateBlock(currentBlock)) {
            if (nextBlockNumber === 2) {
                ...
                // ← tambahkan ini tepat setelah updateFasilitasDropdown()
                if (oldFasilitas) {
                    const dd = document.getElementById('fasilitas-utama-dropdown');
                    // hanya set kalau opsi lama masih ada
                    if ([...dd.options].some(o => o.value === oldFasilitas)) {
                        dd.value = oldFasilitas;
                    }
                }
            }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // muat data lama dari server
        const existingJson = document.getElementById('kebutuhan_data_json').value;
        try {
            kebutuhanDataArray = existingJson ? JSON.parse(existingJson) : [];
        } catch (e) {
            kebutuhanDataArray = [];
        }
